...
public class ScribanCodeGeneratorService(IBaseRepository baseRepository) : BaseService(baseRepository), IScribanCodeGeneratorService
{
    public IEnumerable<string> AllTables(string connectionString, Constants.DbType dbType)
    {
        return GetTablesQuery(connectionString, dbType);
    }
    ...
    private async Task GenerateForTableAsync(string table, InfoClass infoClass)
    {
        ...
        var dbColumns = GetColumnsQuery(infoClass.ConnectionString, infoClass.DbType, table);
