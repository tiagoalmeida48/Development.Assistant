...
public class CompareDatabaseService(IBaseRepository baseRepository) : BaseService(baseRepository), ICompareDatabaseService
{
    ...
    {
        var databaseName = GetDatabaseName(connectionString, dbType);
        var tableNames = GetTablesQuery(connectionString, dbType).ToList();

        var tablesData = await Task.WhenAll(
            tableNames.Select(async tableName =>
            {
                var columnsTask = Task.Run(() => GetColumnsQuery(connectionString, dbType, tableName));
                var countTask = Task.Run(() => GetCountRegistersQuery(connectionString, dbType, tableName));
