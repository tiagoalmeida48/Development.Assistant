...
{
    public static void Initialize(string dllModel, string[] @namespace, string[] excludes)
    {
        var assembly = Assembly.LoadFrom(dllModel);

        var classes = from type in assembly.GetExportedTypes()
            where @namespace.Any(n => type.Namespace.StartsWith(n) && type.IsClass && !excludes.Any(e => type.Name.StartsWith(e)))
            select type;
