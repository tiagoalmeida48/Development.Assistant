...
**Fluxo de request**: Controller (recebe DTO, retorna `OkResult(...)` → `ResultApi<T>`) → Service (regra de negócio) → Repository (Dapper). Exceções sobem para `Api/Middleware/ErrorHandlingMiddleware.cs`, mapeadas por tipo: `UnauthorizedException→401`, `NotFoundException→404`, `BadRequestException→400`, demais→500. **Sempre lance as exceções semânticas de `Shared/Exceptions/`, nunca `throw new Exception(...)`** (genéricas viram 500 e perdem semântica HTTP — há dívida técnica existente assim).

**`ApiContext` (Shared/ApiContext.cs)**: registrado como **Transient** em `Program.cs`, construído a partir do header `Authorization` (decodifica o JWT → `User` = userId, ou 0 se anônimo) + connection string interna. É injetado em `BaseRepository`, deixando o userId logado disponível em toda query (ex.: `input_history WHERE user = @user`).

**Geração de código (coração do projeto)** — `Domain/Services/ScribanCodeGeneratorService.cs`:
1. `CreateClassAsync(InfoClassDto)` processa cada tabela em paralelo (`Task.WhenAll`); dentro de cada tabela, gera os N arquivos também em paralelo.
2. Por tabela: `MetadataRepository.GetColumnsQuery` lê colunas → tipos SQL mapeados para C# por `Constants.GetCSharpType` → monta `DatabaseMetadataVo` → renderiza cada template `.scriban`.
...
**Templates Scriban** (`Infrastructure/Templates/*.scriban`): copiados para `Templates/` no output via `<Content>` no `.csproj` (`PreserveNewest`). Os dados disponíveis dentro do template são definidos por `Constants.MapInfo` (ex.: `table_name`, `columns`, `columns_key`, `namespace`). Prefixo `DDD_*` ou `Clean_*` no nome do arquivo.

**Multi-banco** (`Infrastructure/Repositories/MetadataRepository.cs`): cada operação seleciona o SQL por dialeto via `switch` sobre `dbType` (constantes em `Constants.DbType`); o SQL fica em `Infrastructure/Database/SqlQueries.cs`. Identificadores (nomes de tabela/schema) são interpolados via `string.Format` mas passam por `ValidateIdentifier` (whitelist regex) — ao tocar nessas queries, preserve essa validação.
...
- Sufixos de tipo: `Dto` (Api/Dtos), `Mod` (Domain/Models — "Model"), `Vo` (Domain/ValueObjects).
- **Idioma**: mensagens de erro/logs voltadas ao usuário em **português** (usuários são pt-BR); nomes de código/entidades em inglês.
- Namespace acompanha a pasta física. Mapping entre camadas via `MapTo<T>()` (round-trip JSON, em `Shared/SerializerExt.cs`) — sem AutoMapper.
