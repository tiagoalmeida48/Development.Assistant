...
| `Modules/Common` | Constantes, exceptions, extensions, helpers, `ApiContext`, `ResultApi` (ex-`Shared`) |
| `Modules/Common/Database` | `SqlQueries.cs` (SQL por dialeto) |
| `Modules/Common/Exceptions` | `AppException` e exceções semânticas (401/404/400) |
| `Modules/Common/Extensions` | `AccessExtensions` (extração de token/JWT do `HttpContext`; namespace `Modules.Common.Http`) |
...
Mapping entre camadas via **extension member do C# 14** em `Modules/Common/SerializerExt.cs` (classe `Serializer`):
...
**Evitar**: `throw new Exception("msg")` genérico (use as exceções de `Modules/Common/Exceptions/`); `.Result`/`.Wait()` (use `await`); interpolar input em SQL (use `@param` + `ValidateIdentifier`); `Dictionary` compartilhado entre threads sem `lock`.
