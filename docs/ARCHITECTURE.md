...
Documento técnico único do projeto: camadas, padrões, fluxo de request, geração de código, autenticação, referência da API e convenções de código. A separação de responsabilidades segue um modelo N-tier, mas a **organização física** dos arquivos é **por tipo de artefato** dentro de `Modules/` (`Record`, `Models`, `Services`, `Vo`, `Repository`, `Common`), com `Controllers/` e `Middleware/` na raiz do projeto — ver "Mapa de pastas".
...
1. Middleware / Pipeline
   ...
   • Swagger UI (apenas em Development)
        │
        ▼
2. Controllers  — herdam de BaseController ([ApiController], rota api/[controller]/[action])
   recebem DTO → chamam Service → retornam ResultApi<T> via OkResult(...)
        │
        ▼
3. Modules/Services  — regras de negócio, orquestração, mapping
        │
        ▼
4. Modules/Repository  — Dapper + SQL parametrizado
        │
        ▼
...
A organização é **por tipo de artefato** dentro de `Modules/`; `Controllers/` e `Middleware/` ficam na raiz do projeto.

| Pasta | Responsabilidade |
|-------|------------------|
| `Controllers` | Endpoints HTTP (herdam `BaseController`) — raiz do projeto |
| `Middleware` | `ErrorHandlingMiddleware` — raiz do projeto |
| `Modules/Record` | Contratos de entrada/saída e VOs de transporte (records, sufixo `Record`) |
| `Modules/Models` | Modelos de dados do Dapper (classes, sufixo `Mod`) |
| `Modules/Services` | Casos de uso e regras de negócio |
| `Modules/Services/Templates` | Templates Scriban (`DDD_*`, `Clean_*`) |
| `Modules/Vo` | Value objects de comparação de schemas (`DatabaseClass` e família) |
| `Modules/Repository` | Acesso a dados com Dapper |
| `Modules/Common` | Raiz com `ApiContext`, `Constants`, `ResultApi`, `HashHelper`, `Serializer`, `Exceptions.cs` (ex-`Shared`) |
| `Modules/Common/Exceptions.cs` | `AppException` + exceções semânticas (401/404/400), namespace `Modules.Common` |
| `Modules/Common/Database` | `SqlQueries.cs` (SQL por dialeto) e `DapperMapperHelper.cs` (type mapping do Dapper) |
| `Modules/Common/Extensions` | `StringExtensions` (utils de string) e `AccessExtensions` (token/JWT do `HttpContext`, namespace `Modules.Common.Http`) |

---

## 🧩 Serviços (Modules/Services)
...
Cliente → POST /api/Auth/Login { login, password }
  AuthController.Login  → mapeia LoginRecord para CredentialsRecord (MapTo<CredentialsRecord>)
    AuthService.Login(CredentialsRecord)
...
Todos herdam de `Controllers/_BaseController.cs`:
...
Herança clássica via `Modules/Repository/_BaseRepository.cs`:
...
- `BaseRepository` é fixo em **MySQL** para o banco interno. A introspecção multi-banco vive em `IntrospectionRepository`, que **não herda** de `BaseRepository` (abre conexões dos **bancos-alvo** separadamente via `CreateDbConnection` — ver seção multi-banco). Os metadados do banco interno ficam em repositórios por tabela — `TemplateRepository` (tabela `template`) e `DatabaseTypeRepository` (tabela `database_type`), cada um com um método `All()` e herdando `BaseRepository`.
- Não há interfaces (`IUserRepository`) nem CRUD genérico — cada repo escreve suas queries.
...
Serviços registrados: `AuthService`, `PasswordService`, `UserService`, `MetadataService`, `CompareDatabaseService`, `ScribanCodeGeneratorService`, `CopyProjectService`, `InputHistoryService`. Repositórios: `BaseRepository`, `UserRepository`, `IntrospectionRepository`, `TemplateRepository`, `DatabaseTypeRepository`, `InputHistoryRepository`.

### `ApiContext` (Modules/Common/ApiContext.cs)
...
Mapping entre camadas via **extension member do C# 14** em `Modules/Common/Serializer.cs` (classe `Serializer`):
...
Centralizado em `Middleware/ErrorHandlingMiddleware.cs` (ver pattern matching acima). Resposta sempre no envelope `ResultApi<object>`.

---

## 🎨 Geração de código (pipeline Scriban)

`Modules/Services/ScribanCodeGeneratorService.cs` é o núcleo do projeto.

`CreateClassAsync(InfoClassRecord)`:
1. Para cada tabela **em paralelo** (`Task.WhenAll`):
   - `IntrospectionRepository.GetColumnsQuery` lê colunas da tabela do banco-alvo.
   - Tipos SQL → C# via `Constants.GetCSharpType` (dicionário `Constants.DataType`).
   - Monta `DatabaseMetadataRecord` (record).
...
`Modules/Repository/IntrospectionRepository.cs` seleciona o SQL por dialeto via `switch` sobre `dbType` (constantes em `Constants.DbType`: `MariaDb`, `SqlServer`, `Oracle`, `PostgreSql`). O SQL fica em `Modules/Common/Database/SqlQueries.cs`, organizado por dialeto. A conexão certa é criada por `CreateDbConnection` (`MySqlConnection`/`SqlConnection`/`OracleConnection`/`NpgsqlConnection`).
...
**Extração do userId** (`Modules/Common/Extensions/AccessExtensions.cs`, namespace `Modules.Common.Http`): `GetToken` lê o header `Authorization`; `DecodeJwt` busca o claim `nameid` e retorna o `int` (lança `UnauthorizedException` se ausente/malformado).
...
> Nota: `Modules/Common/HashHelper.cs` é coisa **diferente** — é cifra **simétrica AES reversível** (com IV aleatório prefixado) usada pelo `CryptographyController` para encriptar/decriptar connection strings, não para senhas.
...
|-----------|---------------|-------|------|----------------|---------|
| Auth | `/api/Auth/Login` | POST | 🔓 | `LoginRecord` → `CredentialsRecord` | `AuthService.Login` |
...
| User | `/api/User/Create` | POST | 🔒 | `UserCreateRecord` | `UserService.Create` |
| User | `/api/User/Update` | PUT | 🔒 | `UserUpdateRecord` | `UserService.Update` |
...
| Database | `/api/Database/CompareDatabases` | POST | 🔒 | `ConnectionStringRecord` | `CompareDatabaseService.CompareAsync` |
| CodeGenerator | `/api/CodeGenerator/AllTables` | POST | 🔒 | `ConnectionStringForSearchTablesRecord` | `ScribanCodeGeneratorService.AllTables` |
| CodeGenerator | `/api/CodeGenerator/CreateClass` | POST | 🔒 | `InfoClassRecord` | `ScribanCodeGeneratorService.CreateClassAsync` → **zip** |
| Project | `/api/Project/CopyProject` | POST | 🔒 | `CopyProjectRecord` (paths locais) | `CopyProjectService.CopyProject` |
...
| Cryptography | `/api/Cryptography/Process` | POST | 🔒 | `CryptographyRecord` (`Key`,`Operation`,`Text`) | `HashHelper.Encrypt/DecryptConnectionString` |
| InputHistory | `/api/InputHistory/All?input=&valueInput=&databaseType=` | GET | 🔒 | — (`input` é `[BindRequired]`) | `InputHistoryService.All` |
| InputHistory | `/api/InputHistory/Create` | POST | 🔒 | `CreateInputHistoryRecord` | `InputHistoryService.Create` |
| InputHistory | `/api/InputHistory/Delete?id=` | DELETE | 🔒 | — | `InputHistoryService.Delete` |

Notas:
- `CompareDatabases` aceita `DbType1`/`DbType2` opcionais no `ConnectionStringRecord` (fallback para `DbType`) → compara bancos de tipos diferentes.
...
- Records para DTOs/VOs. **Exceção**: Models do Dapper são **classes com setters públicos** (Dapper popula via reflection) — ex.: `ColumnInfoMod`, que tem `[Column(...)]` e é hidratado por `con.Query<ColumnInfoMod>`.
...
| Sufixo contrato de API | `Record` | `LoginRecord`, `InfoClassRecord` |
| Sufixo Model/Entity | `Mod` | `UserMod`, `TemplateMod`, `ColumnInfoMod` |
| Sufixo Value Object | `Vo` | `DatabaseClass` e família (em `Modules/Vo`) |
...
**Evitar**: `throw new Exception("msg")` genérico (use as exceções de `Modules/Common/Exceptions.cs`); `.Result`/`.Wait()` (use `await`); interpolar input em SQL (use `@param` + `ValidateIdentifier`); `Dictionary` compartilhado entre threads sem `lock`.
...
- `IntrospectionRepository` ainda monta SQL por dialeto com identificadores via `string.Format` (mitigado por `ValidateIdentifier`).
- Possível evolução: extrair interfaces de repositório, trocar `Dictionary`+`lock` por `ConcurrentDictionary` na geração.

---

**Última atualização**: 2026-06-18 (reorganização física por tipo de artefato em `Modules/`; sufixo de contratos `Dto` → `Record`)
